...
module.exports = function (Accessory, Service, Characteristic, HomebridgeAPI, uuid, FakeGatoHistoryService) {
	...
	const { FAHRENHEIT, CELSIUS } = Characteristic.TemperatureDisplayUnits
	const CELSIUS_ABBR = 'C';
	const FAHRENHEIT_ABBR = 'F';
	...
	acAccessory.prototype.getServices = function () {
		...
		this.HeaterCoolerService.getCharacteristic(Characteristic.TargetTemperature).setProps({
			minValue: minTemp,
			maxValue: maxTemp,
			minStep: tempStep
		});
		...
		if (this.capabilities.cool) {
			this.HeaterCoolerService.getCharacteristic(Characteristic.CoolingThresholdTemperature)
				.setProps({
					minValue: this.capabilities.cool.temperatures[CELSIUS_ABBR].values[0],
					maxValue: this.capabilities.cool.temperatures[CELSIUS_ABBR].values[this.capabilities.cool.temperatures[CELSIUS_ABBR].values.length - 1],
					minStep: tempStep
				})
		...
		if (this.capabilities.heat) {
			this.HeaterCoolerService.getCharacteristic(Characteristic.HeatingThresholdTemperature)
				.setProps({
					minValue: this.capabilities.heat.temperatures[CELSIUS_ABBR].values[0],
					maxValue: this.capabilities.heat.temperatures[CELSIUS_ABBR].values[this.capabilities.cool.temperatures[CELSIUS_ABBR].values.length - 1],
					minStep: tempStep
				})
	...
	acAccessory.prototype.getTemperatureDisplayUnits = function (callback) {
		...
		callback(null, this.temperatureUnit === FAHRENHEIT_ABBR ? FAHRENHEIT : CELSIUS)
	}
	...
	acAccessory.prototype.getNormalizedTargetTemperature = () => {
		if (this.acState.temperatureUnit === FAHRENHEIT_ABBR) {
			return fahrenheitToCelsius(this.acState.targetTemperature)
		}
	...
	acAccessory.prototype.usesFahrenheit = () => {
	...
	acAccessory.prototype.updateHomeKit = function (acState, measurements, smartMode) {
		if (measurements) {
			if (this.debug) {
				this.log(this.name, '*New State* - Room Temperature -', measurements.temperature, 'º' + this.temperatureUnit)
